...
def _health_command(job_key, settings):
    details = {
        ...
        "process_snapshot": (
            ...
            {
                "PROCESS_NAMES": powershell_array(settings.diagnostic_process_names),
...
def _network_commands(job_key, target, settings):
    ...
    return (
        _powershell(
            "DNS A and AAAA lookup",
            "dns_resolution.ps1",
            {"HOST_NAME": powershell_literal(target)},
            settings.command_timeout_seconds,
        ),
    )


def build_job_commands(job_key, settings, target=None, minutes=None):
